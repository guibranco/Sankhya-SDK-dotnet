...
internal static class StatusMessageHelper
{
    ...
    /// The common messages
    /// </summary>
    private static readonly Dictionary<string, Func<string, ServiceName, ServiceRequest, ServiceResponse, Exception>> _commonMessages;
        ...
        _commonMessages =
            new()
            {
                ...
                },
                {
                    ...
                    (_, _, request, _) =>
                        new ServiceRequestPartnerStateInscriptionException(request)
                },
                {
    ...
    public static void ProcessStatusMessage(ServiceName service, ServiceRequest request, ServiceResponse response)
    {
        Match match;
        var statusMessage = response.StatusMessage.Value;

        var kvp = _commonMessages.FirstOrDefault(item =>
        ...
        {
            throw new ServiceRequestPropertyValueException(EntityValidation.PropertyValueErrorPattern.Match(statusMessage).Groups[@"propertyName"].Value, request);
        }

        if (EntityValidation.PropertyNameErrorPattern.IsMatch(statusMessage))
        {
            throw new ServiceRequestPropertyNameException(EntityValidation.PropertyNameErrorPattern.Match(statusMessage).Groups[@"propertyName"].Value, request);
        }

        if (EntityValidation.PropertyNameInvalidErrorPattern.IsMatch(statusMessage))
        {
            throw new ServiceRequestPropertyNameException(EntityValidation.PropertyNameInvalidErrorPattern.Match(statusMessage).Groups[@"propertyName"].Value, request);
        }
        ...
        if (EntityValidation.PropertyNameAssociationErrorPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestPropertyNameException(match.Groups[@"propertyName"].Value, match.Groups[@"entity"].Value, request);
        }
        ...
        if (EntityValidation.MissingRelationPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestInvalidRelationException(match.Groups[@"missingRelation"].Value, match.Groups[@"entity"].Value, request);
        }
        if (EntityValidation.PropertyForeignKeyRestrictionPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestForeignKeyException(match.Groups[@"table"].Value, match.Groups[@"column"].Value, request, response);
        }
        if (EntityValidation.DuplicatedDocumentPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestDuplicatedDocumentException(match.Groups[@"name"].Value, request, response);
        }
        if (EntityValidation.BusinessRuleRestrictionPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestBusinessRuleRestrictionException(match.Groups[@"ruleName"].Value, match.Groups[@"errorMessage"].Value, request, response);
        }
        if (EntityValidation.FullTransactionLogsPattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestFullTransactionLogsException(match.Groups[@"database"].Value, request, response);
        }
        if (EntityValidation.MissingAttributePattern.IsMatch(statusMessage))
        {
            ...
            throw new ServiceRequestAttributeException(match.Groups[@"attributeName"].Value, service, request);
        }
        ...
        throw new ServiceRequestPropertyWidthException(match.Groups[@"propertyName"].Value, request, match.Groups[@"widthAllowed"].Value.ToInt32(), match.Groups[@"currentWidth"].Value.ToInt32());
    }
}